...
use intervolga\sed\handler\ContractFileToDealAttachListener;
...
//    const TOPIC_PREFIX = 'TASKS_';
    ...
    public static function updateTaskItem($taskId, $userId, $updateInfo)
    {
        ...
        // В обработчиках события обновления задачи могут возникнуть исключения, игнорируемые при обновлении.
        // Чтобы избежать такого поведения, некоторые из них сохраняются в $APPLICATION.
        global $APPLICATION;
...
//        return static::getUserData(array('ID' => implode(' | ', $defaultUserIds)), array('FIELDS' => array('ID', 'NAME', 'LAST_NAME')));
    ...
    public static function getContractRequestUserFieldCodesList() {
        return array(
                Option::get('intervolga.sed', 'intervolga_sed_contract_deal_code')
        );
    }
}