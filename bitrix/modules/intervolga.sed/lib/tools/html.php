...
class Html
{
    ...
    public static function getTaskDeadlineBlock($deadline, $status) {
        if (empty($deadline) || $status == \CTasks::STATE_COMPLETED) {
            return '';
        }

        $deadline = DateTime::createFrom($deadline)->format('d.m.Y H:i');

        return '<div class="contract-task-deadline">
            <span class = "' . ($status != \CTasks::METASTATE_EXPIRED ? '' : 'expired') . '">' . $deadline . '</span>
        </div>';
    }
}