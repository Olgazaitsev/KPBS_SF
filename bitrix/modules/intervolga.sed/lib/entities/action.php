...
class Action extends TableElement
{
    ...
    /**
     * Возвращает текущее действие SEDа, если оно выполняется.
     *
     * @return mixed|null
     */
    public static function getExecutingAction() {
        return is_null(static::$currentAction) || static::$currentAction->isEmpty() ? null : static::$currentAction->top();
    }
        ...
        if(!empty($actionInfo) && is_array($actionInfo)) {
            foreach ($actionInfo as $actionKey => $actionParams) {
                ...
                if(method_exists(__CLASS__, $action)) {
                    if (is_null(static::$currentAction)) {
                        static::$currentAction = new \SplStack();
                    }
                    static::$currentAction->push($actionKey);
                    static::$action($actionParams, $additionalParams);
                    static::$currentAction->pop();
                }
            }
        }
    }
    ...
    protected static function actionCreateTask($params, $additionalParams)
    {
        ...
        // Название задачи
        if(is_string($params['TASK_TITLE']) && mb_strlen($params['TASK_TITLE'])) {
            $taskParams['TITLE'] = $params['TASK_TITLE'] . Loc::getMessage('C.ACTION.SEPARATOR') . $additionalParams['CONTRACT']['NAME'];
        }
        else {
    ...
    */
    protected static function actionUpdateContractStatus($params, $additionalParams)
    {
        ...
        if($contractId && $newProcessStatus) {
            ...
            // Если завершительные статусы закрываем основную задачу
            if($newProcessStatus == 24 || $newProcessStatus == 25) {
                $contractTask = ContractTask::getOneByFilter(array('CONTRACT_ID' => $contractId, 'IS_MASTER' => 1));
                if($contractTask != null) {
                    $taskId = $contractTask->getId();
                    $updateTaskParams = array();
                    $updateTaskParams['STATUS'] = $additionalParams['TASK']['STATUS'];
                    global $USER;
    ...
    protected static function actionUpdateTask($params, $additionalParams)
    {
        ...
        if($participantsIdList[$originatorRoleId] && $participantsIdList[$responsibleRoleId]) {
            ...
            // Если не указать id статуса при изменении ответственного за задачу,
            // то битрикс сам присвоет ей статус "Принята", что некорректно для задач СЭДа. Учитываем это всегда.
            if (isset($additionalParams['TASK']['STATUS'])) {
                $updateTaskParams['STATUS'] = $additionalParams['TASK']['STATUS'];
            }
