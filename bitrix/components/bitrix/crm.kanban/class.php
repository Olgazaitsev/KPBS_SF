...
class CrmKanbanComponent extends \CBitrixComponent
{
	...
	protected function getItems(array $filter = array(), $pagen = true)
	{
        ...
        self::m_clearAccessResult2($result); // !!!
	 ...
	 * @return void
	 */
	protected function runAutomationOnUpdate(int $id): void
	{
		if (
			$this->type == $this->types['lead'] ||
			$this->type == $this->types['deal']
	...
	{
        self::m_clearAccessResult($this->arResult); // !!!
	...
	// !!!*******************************
    function m_clearAccessResult2(&$result)
    {
        $userId = CUser::GetID();
        ...
        foreach ($FIELDS_ACCESS_DENIED as $item) {
            if (in_array($item["GroupId"], $userGroups) || $isDealFieldsInvisibleEnabled) {
                foreach($result as $k => &$v){
                    if(!in_array($k, $userAssignedDealsIds)) {
                        $v["price"] = 0;
                        $v["price_formatted"] = 0;
                    }
                }
...
// !!!*******************************
    function m_clearAccessResult(&$result)
    {
        if(!is_array($result) || !is_array($result['ITEMS']) || $result['ITEMS']["items"] == null)
            return;
        ...
        foreach ($FIELDS_ACCESS_DENIED as $item) {
            if (in_array($item["GroupId"], $userGroups)) {
                foreach ($result['ITEMS']["items"] as $i => &$item)
                {
                    if(!in_array($item['id'], $userAssignedDealsIds) || $isDealFieldsInvisibleEnabled) {
                        $item["total"] = 0;
                        $item["data"]["price"] = 0;
                        $item["data"]["price_formatted"] = 0;

                        foreach ($item["data"]["fields"] as $f => &$field) {
                            if (in_array($item["UserField"], $field["code"]) || $isDealFieldsInvisibleEnabled) {
                                $field["value"] = 0;
                            }
                        }
                    }
