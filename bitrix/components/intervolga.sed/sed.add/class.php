...
class SedAddComponent extends CBitrixComponent
{
        ...
        if (check_bitrix_sessid() && $this->request->getPost(static::REQUEST_BTN_NAME) == static::REQUEST_BTN_VALUE) {
            if ($this->checkRequestFields()) {
                try {
                    ...
                    $this->saveContractDeadline($this->contractId, $deadline);
                    // !!!!
    ...
    protected function saveContractDeadline($contractId, $deadline){
...
CREATE TABLE IF NOT EXISTS `intervolga_sed_contract_deadline` (
    ...
    PRIMARY KEY (`contract_id`)
        ...
        $sql = "INSERT INTO intervolga_sed_contract_deadline (contract_id, deadline) VALUES (".$contractId.",'".$deadline."')";
        $connection->queryExecute($sql);
