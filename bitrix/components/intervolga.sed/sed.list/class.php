...
//                'ROLE.IS_INITIATOR' => false
        ...
        $taskListData = SedTools\Utils::getTaskData(array('ID' => $taskIdList, '!' . CSEntities\TaskTypeField::TASK_TYPE_FIELD_NAME => false), array('ID', 'CREATED_BY', 'RESPONSIBLE_ID', 'REAL_STATUS', 'DEADLINE', 'STATUS', 'UF_*'));
...
//                'PROCESS_STATUS_ID' => $contract->getReferenceProcessStatusName(),
            ...
            foreach ($roleIdList as $roleId) {
                ...
                if (empty($contactTaskIdList[$roleId]) || empty($taskListData[$contactTaskIdList[$roleId]]['STATUS_NAME'])) {
                    ...
                    $cols['ROLE_' . $roleId] = $taskListData[$contactTaskIdList[$roleId]]['STATUS_NAME'] . SedTools\Html::getTaskDeadlineBlock(
                            $taskListData[$contactTaskIdList[$roleId]]['DEADLINE'],
                            $taskListData[$contactTaskIdList[$roleId]]['STATUS']
                    );
                }
