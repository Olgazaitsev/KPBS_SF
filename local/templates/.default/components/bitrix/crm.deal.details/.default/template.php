...
<?if(!in_array($arResult['ENTITY_ID'], $userAssignedDealsIds) && $arResult['ENTITY_ID'] != 0 || $isDealFieldsInvisibleEnabled):?>

<script type="text/javascript">
	/************Entity_id: <?=$arResult['ENTITY_ID']?>*********<?=!in_array($arResult['ENTITY_ID'], $userAssignedDealsIds) && $arResult['ENTITY_ID'] === 0?>***********************/
    BX.ready( () => {
        var uf = new Object();
        <?

        $executedFields = Array();
        foreach ($FIELDS_ACCESS_DENIED as $item) {
            $display = '';

            if(in_array($item["UserField"], $executedFields))
                continue;

            if (in_array($item["GroupId"], $userGroups) || $isDealFieldsInvisibleEnabled){
                $display = 'none';
                $executedFields[] = $item["UserField"];
            }
            ?>
            /***************GroupId:<?=$item["GroupId"]?>;UserField:<?=$item["UserField"]?>;Display:<?=$display?>******************/
            uf = document.querySelector('[data-cid="<?=$item["UserField"]?>"]');
            if (uf) {
                uf.style.display = '<?=$display?>';
            }
            /*********************************/
        <?
        }
