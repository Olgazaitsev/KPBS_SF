...
<div class="intranet-user-profile" id="intranet-user-profile-wrap">
	...
	?>
        ...
        <div id="intranet-user-profile-tags-container" class="intranet-user-profile-container<?=(empty($arResult["Tags"]["COUNT"]) ? ' intranet-user-profile-container-empty' : '')?>">
            <div class="intranet-user-profile-container-header">
                <div class="intranet-user-profile-container-title">Скрытие полей в сделке</div>
                <script type="application/javascript">
                    function hideOrShowMoneyFieldsInDeal(){
                        BX.ajax({
                            url: '/include/updateUserDealFieldsSettings.php',
                            method: 'POST',
                            onsuccess: function (isHidden) {
                                console.log(isHidden);
                                // let chkBx = BX('checkboxHideOrShowMoneyFieldsInDeal');
                                //if(isHidden)
                                //    chkBx.checked = "checked";

                                //document.getElementById("checkboxHideOrShowMoneyFieldsInDeal").checked = isHidden;
                            },
                            onfailure: function (ex) {
                                alert('Error! ' + ex);
                            }
                        });
                        //   $.get("include/updateUserDealFieldsSettings.php", function(data) { alert("Data Loaded: " + data); });
                    }
                </script>

            </div>
            <? global $USER;
            $isChecked = Utility::IsDealFieldsInvisibleEnabled($USER->GetID());
            ?>
            <input id="checkboxHideOrShowMoneyFieldsInDeal" type="checkbox" <?=$isChecked ? "checked" : ""?> onclick="hideOrShowMoneyFieldsInDeal()">Скрыть поля с деньгами</div>
