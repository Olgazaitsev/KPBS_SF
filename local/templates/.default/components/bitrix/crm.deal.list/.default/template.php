...
foreach ($arResult['DEAL'] as $id => &$value){
    if(!in_array($id, $userAssignedDealsIds) || $isDealFieldsInvisibleEnabled) {
        foreach ($FIELDS_ACCESS_DENIED as $item) {
            if (in_array($item["GroupId"], $userGroups) || $isDealFieldsInvisibleEnabled) {
                if($value[$item["UserField"]])
                    $value[$item["UserField"]] = "0";
            }
        }
    }
}
